...
func ChangeBmlCategoryAssortment(client *websocketserver.Client, data interface{}) {
	...
	// building table
	cat := eventStr["BmlCategory"] 
	table := configcountcategorypage.SelectBmlCategoryAssortmentTable(cat, mongoSession)

	//fmt.Println("category handler check", table)

	// set values to message and send
	var message websocketserver.Message
	message.Name = "bmlCategoryAssortment get"
	message.Data = table
	client.Send <- message
}
